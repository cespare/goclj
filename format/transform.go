...
const (
	...
	// side-effects, require it as vector with a single namespace, and don't use
	// :as or :refer:
	//   [foo]
...
func removeUnusedRequires(ns parse.Node, syms *symbolCache) {
	...
	for i := 0; i < len(children); i++ {
		...
		// As a special case, never remove requires of the form [foo] because they
		// may be being required for their side-effects.
		var keep []parse.Node
